...
Scikit-surgery \cite{scikitsurgery-repo} & ? & ? & ?& ? & 3 & Spyntex & Discussions \\ %4_ADD
3D-Slicer \cite{slicer-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
FAST \cite{fast-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
PlusLib \cite{plus-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
VTK \cite{vtk-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
ITK \cite{itk-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
MITK \cite{mitk-repo} & ? & ? & ?& ? & ? & ? & ? \\ %4_ADD
...
% Documentation for all target groups: (a) users, (b) developers and mainteners, (c) interested parties (e.g. funders). 

% \subsubsection{Maintenace strategy}
% Maintener is the central role of the project.
...
* VTK-WebGPU
% Speeding up rendering on the web
